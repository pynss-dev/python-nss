...
def db_has_cert(options, nickname):
    cmd_args = [CERTUTIL, '-d', options.db_name, '-L', '-n', nickname]
...
def create_ca_cert(options):
    ...
    logging.info(
        "creating ca cert: subject='%s', nickname='%s'",
        options.ca_subject,
        options.ca_nickname,
    )
...
def add_trusted_certs(options):
    ...
    if os.path.isdir(x86_64_multiarch_dir):
        ...
        basedir = os.path.join(os.sep, 'usr', 'lib64')
    module = os.path.join(basedir, 'libnssckbi.so')
    logging.info(
...
def setup_certs(args):
    ...
    parser.add_argument(
        '--verbose', action='store_true', help='provide info level messages'
    )

    parser.add_argument(
        '--debug', action='store_true', help='provide debug level messages'
    )

    parser.add_argument(
        '--quiet', action='store_true', help='do not display any messages'
    )
    ...
    if options.show_certs:
        ...
        for nickname in cert_nicknames:
            logging.info(
                "Certificate nickname '%s'\n%s",
                nickname,
                format_cert(options, nickname),
            )
    ...
    logging.info(
        "client nickname='%s', client subject='%s'",
        options.client_nickname,
        options.client_subject,
    )
