...
def print_extension(level, extension):
    ...
    if oid_tag == nss.SEC_OID_PKCS12_KEY_USAGE:
        ...
        print(
            nss.indented_format(
                nss.make_line_fmt_tuples(level + 1, nss.x509_key_usage(extension.value))
            )
        )
        ...
        print(
            nss.indented_format(
                nss.make_line_fmt_tuples(level + 1, nss.x509_cert_type(extension.value))
            )
        )
        ...
        print(
            nss.indented_format(
                nss.make_line_fmt_tuples(
                    level + 1, extension.value.der_to_hex(nss.OCTETS_PER_LINE_DEFAULT)
                )
            )
        )
        ...
        for i, pt in enumerate(pts):
            ...
            print(
                nss.indented_format([(level + 2, 'Reasons: %s' % (pt.get_reasons(),))])
            )
        ...
        print(
            nss.indented_format(
                nss.make_line_fmt_tuples(
                    level + 2, auth_key_id.key_id.to_hex(nss.OCTETS_PER_LINE_DEFAULT)
                )
            )
        )
        print(
        ...
        nss.SEC_OID_X509_SUBJECT_ALT_NAME,
...
parser = argparse.ArgumentParser(
    description='cert formatting example',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument(
    '-f', '--cert-format', choices=['pem', 'der'], help='format of input cert'
)
parser.add_argument(
    '-p',
    '--print-cert',
...
print(
    nss.indented_format([(2, 'Version: %d (%#x)' % (cert.version + 1, cert.version))])
)
print(
    nss.indented_format(
        [(2, 'Serial Number: %d (%#x)' % (cert.serial_number, cert.serial_number))]
    )
)
...
print(
    nss.indented_format(
        nss.make_line_fmt_tuples(
            2,
            nss.data_to_hex(nss.md5_digest(cert.der_data), nss.OCTETS_PER_LINE_DEFAULT),
        )
    )
)
...
print(
    nss.indented_format(
        nss.make_line_fmt_tuples(
            2,
            nss.data_to_hex(
                nss.sha1_digest(cert.der_data), nss.OCTETS_PER_LINE_DEFAULT
            ),
        )
    )
)
