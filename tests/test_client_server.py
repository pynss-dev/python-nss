...
def auth_certificate_callback(sock, check_sig, is_server, certdb):
    ...
    try:
        # If the cert fails validation it will raise an exception, the errno
...
def client(request):
    ...
    try:
        ...
        print(
            "client: could not resolve host address '%s'" % hostname,
            file=sys.stderr,
        )
        return
    ...
    for net_addr in addr_info:
        ...
        try:
            ...
            sock.close()
            print(
                "client: connection to: %s failed (%s)" % (net_addr, err),
                file=sys.stderr,
            )
...
def cleanup_server(pid):
    try:
        ...
        if err.errno == errno.ECHILD:
            ...
            print("cleanup_server: %s" % err, file=sys.stderr)
